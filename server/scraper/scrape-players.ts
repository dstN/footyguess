...
(async () => {
  ...
  const chunkedPlayers = chunkArray<string>(players, 5);
  ...
  await Promise.all(chunkedPlayers.map(async (chunk) => {
    ...
    for (const name of chunk) {
      ...
      try {
        ...
        const { main_position, secondary_positions } = await page.evaluate(() => {
          ...
          const secondaryNodes = Array.from(document.querySelectorAll(
